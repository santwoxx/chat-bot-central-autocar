  ...
  if (configPath) {
    ...
    if (email && password) {
      ...
      signInWithEmailAndPassword(auth, email, password)
        .then((userCredential) => {
          console.log("[Firebase Auth] Backend de contingência autenticado com sucesso como:", userCredential.user.email);
        })
        .catch((err) => {
          console.warn("[Firebase Auth] Erro ao autenticar backend de contingência:", err.message);
        });
    }
