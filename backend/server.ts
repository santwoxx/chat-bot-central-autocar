...
registerLogCallback((direction, type: "text" | "button" | "template", payload) => {
  ...
  if (localWebhookLogs.length > 500) {
    localWebhookLogs.pop();
  }
...
function enqueueIncomingMessage(msg: QueuedMessage) {
  ...
  if (messageId) {
    if (recentMetaMessageIds.has(messageId)) {
      console.log(`[Queue Engine] Ignorando webhook duplicado: ${messageId}`);
      return;
    }
...
async function processUserMessageQueue(senderPhone: string) {
  try {
    ...
    console.error(`[Queue Queue Error] Erro processando fila para o número ${senderPhone}:`, err);
  } finally {
